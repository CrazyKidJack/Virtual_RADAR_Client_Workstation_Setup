...
SET GITHUB_VERSION_CHK_URL=https://github.com/KSanders7070/Virtual_RADAR_Client_Workstation_Setup/blob/master/Version_Check

SET BATCH_FILE_RELEASES_URL=https://github.com/KSanders7070/Virtual_RADAR_Client_Workstation_Setup/releases


TITLE CONTROL WORKSTATION SETUP (%USER_VER%)
...
echo.
echo.
echo    NOTE- You "MUST" have DROPBOX and the "ZLC PUBLIC" Dropbox folder installed
...
echo.
echo.
echo  Do you have access to the ZLC Public folder and will you allow this Workstation
...
:NAVDATA_CHOICE2_1
	...
	set /p userselected=Type Y or N and Press Enter: 
		...
		IF /I %userselected%==Y GOTO ZLC_DB_COMPARE
		echo.
		echo.
		echo.
		echo.
...
CD /D "%userprofile%\Documents\ATC Workstation Setup CFG"
	IF EXIST "DB_CUSTOM_DIRECTORY.txt" GOTO ZLC_DB_COMPARE1
	GOTO ZLC_DB_COMPARE2

:ZLC_DB_COMPARE1

if exist "%userprofile%\Dropbox\ZLC PUBLIC" SET "DB_DIR=%userprofile%\Dropbox\zlc public"
if exist "%userprofile%\Dropbox\ZLC PUBLIC" echo %DB_DIR%>"%userprofile%\Documents\ATC Workstation Setup CFG\DB_CUSTOM_DIRECTORY.txt"
if exist "%userprofile%\Dropbox\ZLC PUBLIC" GOTO SITES_PREF

	for /f "delims=" %%x in (DB_CUSTOM_DIRECTORY.txt) do set DB_DIR=%%x
	
	SET DB_DIR_CHK=%DB_DIR:~-10%

	IF NOT "%DB_DIR_CHK%"=="zlc public" goto ZLC_DB_COMPARE2
...
IF NOT "%DB_DIR_CHK%"=="zlc public" ECHO *** ERROR *** THERE WAS A PROBLEM WITH YOUR SAVED "ZLC PUBLIC" DIRECTORY. Please Select it again.

echo.
echo.
echo --------------------------------------------------------------------
echo.
echo  Find and Select the "ZLC PUBLIC" Dropbox Folder
echo.
echo     EXAMPLE:
echo.
echo               %userprofile%\Dropbox\zlc public
echo.
echo --------------------------------------------------------------------
echo.
echo.
...
set "psCommand="(new-object -COM 'Shell.Application')^
.BrowseForFolder(0,'Select the ZLC PUBLIC Dropbox Folder',0,0).self.path""
	...
	IF NOT "%DB_DIR_CHK%"=="zlc public" (
			...
			ECHO    In there should be a "ZLC PUBLIC" folder.
			ECHO.
			ECHO    The "ZLC PUBLIC" folder is what you are required to select.
			ECHO.
...
(
ECHO https://zlcartcc.com/index.php/change-log/
ECHO https://www.faa.gov/documentLibrary/media/Order/2019-10-10_Order_JO_7360.1E_Aircraft_Type_Designators_FINAL.pdf
ECHO https://www.aviationweather.gov/taf/data?ids=KBIL+KBOI+KBZN+KSUN+KGPI+KGTF+KHLN+KIDA+KJAC+KTWF+KMSO+KOGD+KPIH+KPVU+KSLC
ECHO https://datis.clowd.io/KBOI
ECHO https://datis.clowd.io/KSLC
ECHO https://www.weather.gov/zlc/predutyweatherbriefing
ECHO https://zlcartcc.com/index.php/position-assumption-or-relief/
)>ZLC_DEFAULT_SITES.txt
...
echo --------------------------------------------------------------------
echo.
echo  The following sites open by default for vZLC ARTCC.
echo.
...
for /f "delims=" %%a in (ZLC_DEFAULT_SITES.txt) do (
...
set /p USE_ZLC_SITES=Type CUSTOM or ZLC and press Enter: 

	IF /I "%USE_ZLC_SITES%"=="CUSTOM" GOTO CUSTOM_SITE_SETUP
	IF /I "%USE_ZLC_SITES%"=="ZLC" GOTO ZLC_SITE_PREF
		ECHO.
		ECHO.
		ECHO.
		ECHO "%USE_ZLC_SITES%" IS NOT A RECOGNIZED RESPONSE. TRY AGAIN.
		PAUSE
		CLS
		GOTO SITES_PREF
...
ECHO USE_ZLC_SITES>USER_SITES_PREF.txt
...
IF EXIST "%userprofile%\Dropbox\zlc public\Other Downloads\ZLC_LUFL.bat" GOTO LUFL_START
	GOTO VATSPY_STRT

:LUFL_START

CD /d "%userprofile%\Dropbox\zlc public\Other Downloads"
	START /MAX ZLC_LUFL.bat
...
IF "%USER_SITES_PREF%"=="USE_ZLC_SITES" SET SITE_SOURCE=ZLC_DEFAULT_SITES.txt
...
PING 127.0.0.1 -n 2 >nul
CD /D %AFV_DIR%
cmd /min /C "set __COMPAT_LAYER=RUNASINVOKER && start "" "AudioForVATSIM.exe""
PING 127.0.0.1 -n 2 >nul
...
CD /D "%DB_DIR%\ZLC Facility Files (Live Update)\vSTARS"
...
CD /D "%DB_DIR%\ZLC Facility Files (Live Update)\vERAM"
...
@REM USER SELECTS THE LETTER OF THEIR ISSUE AND THE BATCH FILE SKIPS TO THAT FUNCTION.
	...
	set /p userselected=Type The Letter of Your Response and Press Enter: 
...
ECHO.
ECHO.
ECHO  IF YOU USE ZLC STANDARD SITES:
